...
#' @param ms a list of shapes (eg as returned by \link{meanshapes})
...
plot_meanshapes <- function(x, size=3/4, col2="#FF0000"){
  ms <- x
  # so that the result of meanshapes can directly be passed
  ...
  n <- length(ms)
  plot(NA, xlim=c(0.5, n+0.5), ylim=c(0.5, n+0.5), axes=F, ann=F)
