...
#' @param fac the id or column name in $fac to use for colors and mono. Note that
...
#' @param tip_fac the id or column name in $fac to use as tip_labels rather than rownames. Note that
...
#' CLUST(bp)
#' CLUST(bp, layout="rectangular") + xlim(c(NA, 0.15))
#' CLUST(bp, layout="slanted") + xlim(c(NA, 0.15))
#' CLUST(bp, layout="unrooted") +  xlim(c(NA, 0.15))
...
#' @rdname CLUST
#' @export
# CLUST.PCA <- function(x, fac, layout="unrooted",
...
#     }
#   if (!is.null(tip_fac)) {
#     if (length(tip_fac) == nrow(x$x)) {
#       rownames(x$x) <- as.character(tip_fac)
#     } else {
#
...
CLUST.PCA <- function(x,
  ...
  nr <- nrow(x$x)
  phylo <- dist(x$x, method = dist_method) %>% hclust(method = hclust_method) %>% ape::as.phylo()
  phylo_df <- fortify(phylo)
  phylo_df %<>% mutate(fac=rep(1, nrow(phylo_df)))
  if (!missing(fac))
    phylo_df$fac <- factor(c(as.character(x$fac[, fac]), rep(NA, nrow(phylo_df)-nr)))
  if (!is.null(abbreviate))
