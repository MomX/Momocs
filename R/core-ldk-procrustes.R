...
#' @keywords Procrustes
#' @export
fgProcrustes <- function(x, tol, verbose) {
    UseMethod("fgProcrustes")
}

#' @export
fgProcrustes.default <- function(x, tol = 1e-05, verbose = TRUE) {
    A <- x
    A <- ldk.check(A)
    # directly borrowed from Claude
    p <- dim(A)[1]
    ...
    while (abs(Q) > tol) {
        ...
        M <- mshape(temp1)
        for (i in 1:n) {
            sf[i] <- sqrt(sum(diag(temp1[, , i] %*% t(M)))/(sum(diag(M %*% 
                t(M))) * sum(diag(temp1[, , i] %*% t(temp1[, 
                , i])))))
        ...
        iter <- iter + 1
        if (verbose) {
            cat("iteration: ", iter, "\tgain:", signif(abs(Q), 
                5), "\n")
        }
        temp1 <- temp2
    }
    list(rotated = temp2, iterationnumber = iter, Q = Q, Qi = Qi, 
        Qd = Qd, intereuclidean.dist = Qm2, mshape = coo.centsize(mshape(temp2)), 
        cent.size = Siz)
}
