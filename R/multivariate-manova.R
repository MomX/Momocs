...
#' Manova(op, "domes")
#' @export
Manova <- function(x, fac, test="Hotelling", retain, drop, verbose){UseMethod("Manova")}

#' @rdname Manova
#' @export
Manova.OpnCoe <- function(x, fac, test="Hotelling", retain, drop, verbose=TRUE){
  OpnCoe <- x
  ...
  keep <- (drop+1):retain
  if(verbose) cat(" * Manova done on:", colnames(x)[keep], "\n")
...
Manova.OutCoe <- function(x, fac, test="Hotelling", retain, drop, verbose=TRUE){
  ...
  if (OutCoe$method == "eFourier"){
    ...
    if (sum(x[, BC])==0){
      x <- x[, -BC]
      cph <- 2
      if(verbose) cat(" * B and C harmonics removed (because of removeAsymetric)\n")
    } else {
      if (sum(x[, AD])==0){
        x <- x[, -AD]
        cph <- 2
        if(verbose) cat(" * A and D harmonics removed (because of removeSymetric)\n")
      }
    }
  }
  ...
  if (missing(drop)) {
    if (OutCoe$norm) {
      drop <- 1
      if(verbose) cat(" * 1st harmonic removed (because of normalization)\n")
    } else {
      drop <- 0 }}
  ...
  if (missing(retain)) {
    ...
    if(verbose) {cat(" * 'retain' was missing. MANOVA done with", retain, "harmonics ")
                 if (drop>0) { cat( "and the first", drop, "dropped.\n") } else {cat("./n")}}
  } else {
    if ((retain - drop)> max.h) {
      retain <- max.h + drop
      if (retain > nb.h) { retain <- nb.h }
      if(verbose) cat(" * 'retain' was too ambitious. MANOVA done with", retain, "harmonics ")
      if (verbose) {
        if (drop>0) {cat( "and the first", drop, "dropped.\n")} else {cat("./n")}}}}
...
#' @note If the fac passed has only two levels, there is only pair and it is 
#' equivalent to \link{Manova}.
#' @seealso \link{Manova}
#' @keywords Multivariate
...
#' @rdname ManovaPW
#' @export
ManovaPW <- function(Coe, fac, verbose, ...){UseMethod("ManovaPW")}
#' @rdname ManovaPW
#' @export
ManovaPW.Coe <- function(Coe, fac, verbose=FALSE, ...){
  # preliminaries
  fac0 <- fac
  if (length(Coe$method)>1) stop(" * cannot yet be used on combined OutCoe. Do it manually.")
  if (missing(fac)) stop("'fac' must be provided")
  if (!is.factor(fac)) {fac <- Coe$fac[, fac]}
  ...
  # we prepare a 'signifance' table, with 'significant' stars (discutable but helpful)
  # see stats:::print.summary.lm
  stars <- symnum(res[, 6], corr = FALSE, na = FALSE, 
                  cutpoints = c(0, 0.001, 0.01, 0.05, 0.1, 1), 
                  symbols = c("***", "**", "*", ".", "-"))
  stars <- as.character(stars)
  nl <- nlevels(fac)
  # bloody ugly
  stars.tab <- matrix(NA, nrow=nl-1, ncol=nl, dimnames=list(levels(fac)[-nl], levels(fac)))
