...
print.OpnCoe <- function(x, ...) {
    ...
    cat(" - registered on the baseline: [(", OpnCoe$baseline1[1], 
        "; ", OpnCoe$baseline1[2], ") - (", OpnCoe$baseline2[1], 
        "; ", OpnCoe$baseline2[2], ")]\n", sep = "")
    ...
    cat("etc.\n")
    cat(" - r2 summary ($r2) - min, median, max: ", signif(c(min(OpnCoe$r2), 
        median(OpnCoe$r2), max(OpnCoe$r2)), 3), "\n")
    # number of grouping factors
    df <- OpnCoe$fac
    nf <- ncol(df)
    if (nf == 0) {
