...
a `matrix` of (x; y) coordinates, or any
[Coo](http://momx.github.io/Momocs/reference/Coo.md) object.

## Details

If some shapes are upside-down (or mirror of each others), try
redefining a new starting point (eg with coo_slidedirection) before the
alignment step. This may solve your problem because coo_calliper orders
the `$arr.ind` used by coo_aligncalliper.
