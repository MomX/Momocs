...
`coo_template` returns shape centered on the origin and inscribed in a
`size`-side square. `coo_template_relatively` does the same but the
biggest shape (as `prod(coo_diffrange)`) will be of `size=size` and
consequently not defined on single shapes.
...
for an illustration of this function. The morphospaces functions also
take profit of this function. May be useful to develop other graphical
functions.
