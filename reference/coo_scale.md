...
assumed to be the centroid size. It involves three steps: centering from
current position, dividing coordinates by 'scale', pushing back to the
original position. `coo_scalex` applies a scaling (or shrinking)
parallel to the x-axis, `coo_scaley` does the same for the y axis.
...
- scale:
  ...
  the scaling factor, by default, the centroid size for `coo_scale`; 1
  for `scalex` and `scaley`.
