...
Registers a new baseline for the shape, with the `ldk1`-th and `ldk2`-th
points being set on \\(x= -0.5; y=0)\\ and \\(x= 0.5; y=0)\\,
respectively.
...
- ldk1:
  ...
  `numeric` the id of the first point of the new baseline (the first, by
  default)

- ldk2:

  `numeric` the id of the second point of the new baseline (the last, by
  default)
...
For [Out](http://momx.github.io/Momocs/reference/Out.md), it tries to do
it using `$ldk` slot. Also the case for
[Opn](http://momx.github.io/Momocs/reference/Opn.md), but if no landmark
is defined, it will do it on the first and the last point of the shape.

For `Out` and `Opn` defines the first landmark as the first point of the
new shapes with
