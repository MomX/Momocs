...
test_that("Coo works fine", {
  ...
  expect_true(x %>% Opn() %>% validate %>% is_Coo())
  expect_true(x %>% Ldk() %>% validate %>% is_Coo())
})

test_that("Out works fine", {
  expect_true(x %>% validate %>% is_Out())
