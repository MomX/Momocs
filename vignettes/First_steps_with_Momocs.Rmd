...
It introduces/redefines some concepts:
* A *shape* is an ordinate set (id varies from 1 to shape_length) of 2D (so far) coordinates
* Collections or `Coo`dinates are shapes + (classifiers and/or covariates) + hypotheses + questions.
* `Coo` objects are turned into `Coe`fficients with appropriate morphometric methods, and on which to apply multivariate analyses. A lot of intermediate steps can be done, but the overall architecture is as follows:
...
`CAPITALS`    | Multivariate method       | `PCA`
