...
The Conte (see `?import_Conte`) algorithm extracts the image outline through an iterative process that is initialized at the image center. If it does not find a black pixel at center of the image, a `locator(1)` can be called and asks you to click a point within the shape, or another random point is selected.
...
coordinates <- import_jpg(lf)
coordinates
# alternatively
coordinates <- import_jpg() # now select a file
```

Above, `coordinates <- import_jpg()` will invite you to choose interactively any `jpg` in the folder, and will extract all outlines. But it is less elegant, as your script will require an action to be executed.
